...
2. **Confirme autenticação** tentando uma chamada leve ao MCP
   ...
   invioláveis — MCP tools `auth_login_start` → magic link no e-mail →
   `auth_login_poll` até confirmação. Nunca instrua o usuário a autenticar
   por CLI (o binário `conversion` foi descontinuado).
